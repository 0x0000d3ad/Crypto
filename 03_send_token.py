...
def get_balances( web3, account, args.to_address, contract ) :
    balance_from = blockutil.get_token_balance( web3, account[ "address" ] )
    balance_to = blockutil.get_token_balance( web3, to_address )
...
if __name__ == "__main__" :
    ...
    if args.send_eth :
        ...
        contract = get_contract( web3, contract_address, abi, verbose=verbose )

        temp = get_balances( web3, account, args.to_address )
        print( "Balances before:" )
        print( json.dumps( temp, indent=2 ) )

        transaction_id = blockutil.send_token( web3, chain_id, account, args.to_address, float( args.ether_amount ), verbose=args.verbose ) 
